...
def main(cfg):
    ...
    print(f"input_trans_cls: {input_trans_cls}")
    print(f"input_trans_args: {input_trans_args}")
    print(f"target_trans_cls: {target_trans_cls}")
    print(f"target_trans_args: {target_trans_args}")
...
def evaluate_model(module, data_module, cfg_evaluation, input_shape, logger, input_transformer, target_transformer):
    ...
    for dataloader_index, dataloader in tqdm(enumerate(dataloaders), total=len(dataloaders)):
        ...
        print(f"changes_all: {changes_all.shape}")
        print(f"embs_all: {embs_all.shape}")
        print(f"recons: {recons.shape}")
        print(f"batch_test: {len(batch_test)}")
        print(f"batch_test shape: {batch_test[0].shape}")
        ...
        print(f"batch_test: {len(batch_test)}")
        print(f"batch_test shape: {batch_test[0].shape}")
        print(f"batch_test shape: {batch_test[1].shape}")
        print(f"detection_methods: {detection_methods}")
        print(f"summary_stat_methods: {summary_stat_methods}")
        print(f"is_vae: {is_vae}")
        ...
        data_all_file = open('data_all', 'ab')
        pickle.dump(data_all, data_all_file)                    
        data_all_file.close()
        changes_all_file = open('changes_all', 'ab')
        pickle.dump(changes_all, changes_all_file)                    
        changes_all_file.close()
        embs_all_file = open('embs_all', 'ab')
        pickle.dump(embs_all, embs_all_file)                    
        embs_all_file.close()  
        recons_file = open('recons', 'ab')
        pickle.dump(recons, recons_file)                    
        recons_file.close()  
        ...
        # dump variables
        detection_method_file = open('detection_method_image_names', 'ab')
        pickle.dump(detection_method_image_names, detection_method_file)                    
        detection_method_file.close()

        change_results_file = open('change_results', 'ab')
        pickle.dump(change_results, change_results_file)                    
        change_results_file.close()  

        changes_in_this_location_file = open('changes_in_this_location', 'ab')
        pickle.dump(changes_in_this_location, changes_in_this_location_file)                    
        changes_in_this_location_file.close() 

        results_file = open('results', 'ab')
        pickle.dump(results, results_file)                    
        results_file.close()   

        #summary_stat_methods_file = open('summary_stat_methods', 'ab')
        #pickle.dump(summary_stat_methods, summary_stat_methods_file)                    
        #summary_stat_methods_file.close()        
        ...
        if save_plots_locally:
            ...
            print(f"detection_method: {detection_method_image_names}")
            print(f"change_ids: {change_ids}")
            print(f"data_all shape: {data_all.shape}")
            print(f"changes_all shape: {changes_all.shape}")
            print(f"change_index: {change_index}")
            print(f"changes_in_this_location: {len(changes_in_this_location)}")
            print(f"changes_in_this_location 0 shape: {changes_in_this_location[0].shape}")
            print(f"change_results: {len(change_results)}")
            print(f"change_results 0 shape: {change_results[0].shape}")
